...
class ColorDescriptor:
    ...
    def describe(self, image):
        ...
        cv2.ellipse(ellipMask, (cX, cY), (axesX, axesY),
                    0, 0, 360, 255, -1)
        ...
        for (startX, endX, startY, endY) in segments:
            ...
            cv2.rectangle(cornerMask, (startX, startY),
                          (endX, endY), 255, -1)
            cornerMask = cv2.bitwise_and(
                cornerMask, cv2.bitwise_not(ellipMask))

            # extract histogram and update feature vector
            hist = self.histogram(image, cornerMask)
            features.extend(hist)

        # extract histogram from elliptical region
    ...
    def histogram(self, image, mask):
        ...
        # normalize the histogram (OpenCV 4.x compatible)
